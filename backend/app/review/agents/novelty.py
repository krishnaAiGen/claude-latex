"""Novelty dimension agent — identifies overlap with prior work."""
...
logger = logging.getLogger("review.agents.novelty")

SYSTEM_PROMPT = """You are a specialist peer reviewer focused on NOVELTY.
Your task: identify where the paper's contribution overlaps with or is subsumed by prior work,
given the benchmark papers below.

For each issue, output a JSON array with objects matching this schema:
{
  "severity": "critical|major|minor|nit",
  "title": "one-line headline (max 80 chars)",
  "section": "section name (e.g. Introduction, Related Work, Method)",
  "line": null or integer,
  "body": "2-3 sentence explanation with evidence",
  "fix": "concrete suggested change",
  "relatedBenchmarks": ["B3", "B12"],
  "confidence": 0.0-1.0
}

Rules:
- Output ONLY the JSON array, no prose, no markdown fences.
- Produce 3-6 findings total. Prioritize critical/major over minor/nit.
- If the paper is genuinely novel, produce 1-2 minor findings at most.
- relatedBenchmarks must reference IDs from the benchmark list (B1, B2, ...).
"""
...
def _format_benchmarks(benchmarks: list[BenchmarkPaper]) -> str:
    lines = []
    for b in benchmarks:
        lines.append(f"[{b.id}] {b.title} ({b.authors}, {b.venue} {b.year}) — {b.citations} citations")
    return "\n".join(lines)


async def run_novelty_agent(
    latex: str,
    ...
    if rlog:
        rlog.log_agent_start("novelty", len(paper_excerpt), len(benchmarks))
        rlog.log_agent_prompt("novelty", system_prompt, user_prompt)
    ...
    if rlog:
        rlog.log_agent_response("novelty", response.content, elapsed)
    ...
    if rlog:
        rlog.log_agent_findings("novelty", findings)
    return findings
